...
for (const [route, text] of Object.entries(pageText)) {
  assert.match(
    text,
    /Free, open-source flight boards for screens you own\. No account or subscription required\./,
    `${route} must carry the shared ownership promise`,
  );
  ...
  const navigation = builtPages.get(route)?.match(/<nav\b[^>]*data-site-nav[\s\S]*?<\/nav>/)?.[0] || "";
...
assert.match(pageText["index.html"], /Open-source flight boards\. Built for your screens\./);
assert.match(pageText["index.html"], /Free to download · No subscription · No account required · Optional online service/);
assert.match(pageText["index.html"], /Beacon Tools’ shared online service \(Community Relay\)/);
...
for (const disallowedTerm of ["bounded", "cadence", "envelope", "surface", "platform-wide line"]) {
...
assert.match(relayText, /Community Relay serves Local Flight apps/);
