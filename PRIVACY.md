...
The 0.7.1 candidate retains these data boundaries. Described purchase and
...
- The optional shared-data service can provide recent real-flight schedules without putting provider keys on every device. Shared real-aircraft radar is included; BYOK and VATSIM radar also remain available.
...
If you choose **Beacon Relay**, the Beacon Tools connection service can provide a recent shared copy of real-world schedules. This protects provider keys and lets several Local Flight installs reuse one safe result instead of making the same paid request repeatedly. Shared real-aircraft radar is included in Relay Access, so a radar request also sends the airport coordinates and range being viewed; it also remains available through appropriately licensed BYOK sources or VATSIM. Which commercial sources and capabilities can be enabled remains controlled separately by verified provider permissions; paying for Relay Access never overrides a provider agreement.
...
Mobile Standalone uses the same hosted relay but with stricter product limits: real airline-schedule refresh is 1 hour minimum. Shared real-aircraft radar is available with Relay Access at a 3-minute minimum refresh and a monthly allowance. VATSIM radar uses sanitized virtual-data endpoints, ranges of `1`, `3`, `5`, and `10` NM, and needs no Relay Access credential.
...
| ADS-B Exchange / JETNET | Direct path: API key and radar search coordinates. Relay-backed path: the hosted relay makes the upstream request with its own provider key and shared cache, under a provider agreement that expressly permits it. | [jetnet.com/privacy-policy](https://www.jetnet.com/legal/privacy-policy), [rapidapi.com/privacy](https://rapidapi.com/privacy/) |
...
| UptimeRobot | Checks that the public website and relay endpoints answer, from outside Beacon Tools hosting, so outages are noticed. It receives no information about website visitors: the checks are made against public endpoints on a fixed schedule, and the service-status page reads the result through the Beacon Tools website rather than from your browser. | [uptimerobot.com/privacy](https://uptimerobot.com/privacy/) |
