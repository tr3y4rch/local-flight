...
export function useLaunchOverlay(onHydrated: (value: LaunchHydration) => void) {
  ...
  }, [opacity, reduceMotion, scale, shift]);
  ...
  useEffect(() => {
    ...
    const beaconAnim = Animated.loop(
  ...
  }, [beacon, enter, onHydrated, orbitFast, orbitMedium, orbitSlow, progress, pulse, reduceMotion, sweep]);
  ...
  return {
    ...
    ready,
    enter,
    status: ready ? "Ready for board" : LAUNCH_STATUS_STEPS[statusIndex] ?? "Opening mobile"
  };
}
