...
assert.equal(app.ios.buildNumber, "14");
assert.equal(app.android.versionCode, 17);
...
const iosMediumWidget = read("native/ios-widget/MediumWidgetViewV2.swift");
...
assert.match(iosSnapshot, /\.withFractionalSeconds/);
assert.match(iosSnapshot, /fractional\.date\(from: value\) \?\? standard\.date\(from: value\)/);
assert.match(iosSnapshot, /widgetAppearance: String\?/);
assert.match(iosSnapshot, /liveActivityAppearance: String\?/);
assert.match(iosSnapshot, /rawTime\.split\(separator: " ", maxSplits: 1\)/);
assert.match(iosWidget, /localflight:\/\/board\?source=widget/);
...
assert.match(iosLiveActivity, /Text\(state\.statusDisplay\)/);
...
assert.doesNotMatch(iosLiveActivity, /state\.stale \? "Stale" : state\.statusDisplay/);
assert.match(iosLiveActivity, /accessibilityLabel\(state\.stale \? "\\\(state\.statusDisplay\), cached information"/);
assert.match(iosLiveActivity, /localFlightActivityURL\(context\.attributes\.flightID\)/);
assert.match(iosLiveActivity, /URLQueryItem\(name: "pin", value: flightID\)/);
assert.match(iosLiveActivity, /DynamicIslandExpandedRegion/);
assert.match(iosLiveActivity, /state\.appearance == "light"/);
assert.match(iosLiveActivity, /compactLeading:/);
assert.match(iosLiveActivity, /minimal:/);
assert.match(iosMediumWidget, /\.padding\(\.top, 16\)/);
assert.match(iosMediumWidget, /minimumScaleFactor\(0\.54\)/);
assert.match(iosMediumWidget, /\.frame\(width: 50, alignment: \.leading\)/);
...
assert.match(androidProvider, /localflight:\/\/board\?source=widget&refresh=1/);
...
assert.match(androidProvider, /widget_row_3/);
assert.doesNotMatch(androidProvider, /widget_row_4/);
assert.match(androidProvider, /widgetAppearance/);
assert.match(androidProvider, /resolvePalette/);
assert.match(androidProvider, /substringBefore\(" "\)/);
assert.match(androidLayout, /@\+id\/widget_row_1_time/);
assert.match(androidLayout, /@\+id\/widget_row_1_route/);
assert.doesNotMatch(androidLayout, /localflight_widget_board_columns/);
...
assert.match(widgetBridge, /probeLocalFlightWidgetSnapshot/);
assert.match(widgetBridge, /writeLocalFlightWidgetSnapshot/);
...
assert.match(widgetBridgeApple, /AsyncFunction\("probeSnapshot"\)/);
assert.match(widgetBridgeApple, /AsyncFunction\("writeSnapshot"\)/);
assert.match(widgetBridgeApple, /options: \.atomic/);
assert.match(widgetBridgeApple, /"decodeResult"/);
assert.match(widgetBridgeApple, /"rowCount"/);
assert.match(widgetBridgeApple, /"pinPresent"/);
assert.match(liveActivityManager, /LocalFlightBridgeISO8601\.date/);
...
assert.match(liveActivityManager, /lhs\.flightID == rhs\.flightID/);
assert.doesNotMatch(liveActivityManager, /lhs\.routeName|lhs\.displayTime/);
