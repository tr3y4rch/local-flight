...
assert.match(metadata.fullDescription, /optional annual Beacon Relay Access subscription/i);
assert.match(metadata.fullDescription, /one phone using real-flight Standalone or one Local Flight desktop/);
assert.match(metadata.fullDescription, /Restore checks existing annual or eligible founder access before offering a purchase/is);
