...
def _number_for_airline(identifier: str, airline: dict[str, Any]) -> str:
    compact = _compact_identifier(identifier)
    ...
    if match and icao and match.group(1) == icao:
        return f"{iata or icao}{match.group(2)}"
    return compact
...
def _identifier_for_airline(
    ...
    for identifier in identifiers:
        if _identifier_matches_airline(identifier, airline):
            return _number_for_airline(identifier, airline)
    if _identifier_matches_airline(callsign, airline):
        return _number_for_airline(callsign, airline)
    return None
...
def resolve_flight_identity(
    ...
    *,
    ...
    marketing_flight = _compact_identifier(record.get("marketing_flight_number") or record.get("flight_number"))
    raw_codeshares = _identifier_list(record.get("codeshares"))
    raw_sold_as = _identifier_list(record.get("sold_as"))
    ...
    primary_flight = _identifier_for_airline(operating_airline, list(identifiers), callsign) or marketing_flight or callsign or None
