...
def _recent_fetch_metrics(default_dates_touched: int) -> Dict[str, Any]:
    ...
    if not isinstance(history, list) or not history:
        return {
            "samples": 0,
            "avg_pages_per_direction": float(max(1, default_dates_touched)),
...
def fetch_flights_strategy(
    *,
    ...
    combined, meta = _fetch_request_plan(requests_plan, timeout_s=timeout_s)
    meta.update(
        {
            "strategy": strategy,
...
def estimate_schedule_cost(source: Optional[str] = None) -> Dict[str, Any]:
    ...
    avg_pages_per_direction = min(
        ...
        max(float(dates_touched), float(recent.get("avg_pages_per_direction", dates_touched) or dates_touched)),
    )
