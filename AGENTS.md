...
## Current 0.5.1 handoff snapshot (2026-07-12)
...
- Desktop and Pi are the public release path for this milestone. Mobile is store-bound after the Remote Companion connectivity/privacy proof pass, with Beacon-owned bundle/package IDs `cc.beacontools.localflight`, URL scheme `localflight`, and widget-enabled source targets iOS build number `5` and Android versionCode `9` for the next `0.5.1` testing upload.
...
- Manual Phase 2 smoke still remains before release artifacts: fresh install/upgrade on Windows, ad-hoc-signed macOS zip, and Raspberry Pi; Pi headless/Chromium kiosk/native kiosk services; physical Matrix/i75W script generation and live gate/weather toggles; and live real/VATSIM/BYOK/community/managed/virtual mode walkthroughs against the actual relay/provider setup.
- The current public macOS path intentionally uses an ad-hoc-signed Apple silicon app archive while Developer ID credentials are unavailable. Current `dist/LocalFlight-0.5.1-macos.zip` SHA256 is `abfb8eb97a98bdd9aa89401fdb4ddfeba9b7ac0c90c308bf7e0e79392d890047`; users extract it, move `LocalFlight.app` to Applications, and use Finder Open once. Do not describe it as notarized, and never recommend disabling Gatekeeper globally. The signed/notarized `.pkg` builder remains available as a later upgrade.
- Current `dist/LocalFlight-pi-source-0.5.1.zip` SHA256 is `325b9e3e6dc1265993f7e3a856837c1dcfefc53362a023ef70b3e11aea851ee0`. Both artifacts were rebuilt from the July 13 working tree and must be rebuilt again if any included source or public documentation changes before release.
- Beacon Tools exposes `GET /api/releases/latest` from the Cloudflare Worker as a cached, sanitized GitHub Releases manifest. The public `/local-flight/#downloads` cards activate only for exact release-version Windows `.exe`, macOS app `.zip`, or Pi `.zip` names with matching `.sha256` assets under this repository's GitHub download path. Files remain hosted by GitHub; API failure or missing assets fall back to the Releases page. Run `node scripts/site_downloads_contract.mjs` before deploying site changes.
- The current mobile build implements three optional consumable support products through `expo-iap`: `cc.beacontools.localflight.support.small|medium|large`. They unlock nothing and have no durable entitlement. `/v1/mobile/iap/verify` validates Apple transactions or Google purchase tokens before the app finishes/consumes them; the relay ledger stores only keyed hashes and coarse status metadata. Store product creation, agreements/tax setup, Fly verification secrets, and real TestFlight/Play license testing remain release gates.
- IAP implementation validation on macOS is green at source/native level: `555 passed`, mobile `verify`/`a11y`/Expo Doctor 18/18, Android `:app:compileDebugKotlin`, and an arm64 iOS Simulator `xcodebuild` with `ExpoIap`, `openiap`, and the WidgetKit extension. The first universal simulator attempt exhausted local disk; after generated-cache cleanup and Codegen regeneration the arm64 build succeeded. No real purchase can work until store products and relay credentials exist, and the relay must be redeployed after those secrets are configured.
- Home-screen widgets are now native on both platforms: `mobile/native/ios-widget/` is wired through `with-localflight-ios-widget`, and `mobile/native/android-widget/` is wired through `with-localflight-android-widget`. Both consume only the app-written `localflight-widget-snapshot.json`, enforce a 64 KiB read cap, and must remain network-free. iOS `app.json` explicitly declares the App Group and `extra.eas.build.experimental.ios.appExtensions` credentials metadata; preserve both or EAS may discover the extension too late to sign it. Dynamic Island/ActivityKit remains deferred.
- Packaged desktop provider keys now resolve to `~/.localflight/.env` and are written atomically with user-only POSIX permissions instead of modifying `Local Flight.app/Contents/.env`. Source checkouts continue to use the repository `.env`. Preserve the frozen-path regression test before signing/notarizing any macOS package.
- macOS direct-download packaging is versioned: `python build.py --clean` creates `dist/LocalFlight.app`, `dist/LocalFlight-0.5.1-macos.zip`, and its checksum. The zip is the current website/GitHub path until Developer ID credentials permit the optional signed/notarized `.pkg`. Do not reuse the older unversioned `LocalFlight-macos.zip` or stale pre-`0.5.1` handoff notes.
...
- Previous mobile `0.5.1` signed beta builds completed on EAS from commit `4fd46ad`: iOS build `4` is build ID `1adc2a18-2997-4c5f-ba6d-9052e610e269`; Android versionCode `8` is build ID `b0d2f061-7626-4010-a884-822edecd4a3f`. Widget-enabled source now targets build `5` / `9` and requires fresh native EAS builds. Submission still needs one-time store linkage: add the Local Flight App Store Connect numeric `ascAppId` to the EAS submit profile after signing into App Store Connect, and configure the Google Play service-account key through EAS/Play Console. Never commit either store credential or service-account JSON.
- Separation of power still applies: public docs stay user-focused; relay operator/admin details belong only in `AGENTS.md`, `DEV_README.md`, `CLAUDE.md`, and operator tooling. Cloudflare Worker + Assets serves the no-build public site from `site/`; edits there only go live after a real Worker deploy from the repo, not from a dashboard preview alone. Public pages may mention that admin tooling is separate, but must not expose credentials, Fly secrets, raw admin API details, private admin URLs, Linear team IDs, or internal runbooks.
- July 12 relay/site deployment state: code and copy are ready, the Cloudflare Worker + 48 assets passes Wrangler 4.110.0 dry-run, and live health/pages still respond from the previous deployment. Production deploy was not attempted because both local CLI sessions require fresh browser authorization (`flyctl auth login`; `npx --yes wrangler@4.110.0 login`). After login, deploy from the committed clean tree with `flyctl deploy --remote-only --config relay/fly.toml --dockerfile relay/Dockerfile --ignorefile .gitignore` and `npx --yes wrangler@4.110.0 deploy --config wrangler.jsonc`, then verify health and all public trust pages.
...
### API call budget
- Local direct-provider budgets are enforced before outbound calls. New code uses SQLite counters in `~/.localflight/api_usage.sqlite3` and keeps legacy `~/.localflight/api_usage.json` readable for display/backward compatibility.
...
- Mobile lives in `mobile/` as one React Native / Expo app for iOS and Android, with two first-run setup modes: `lan_companion` and `standalone`.
...
- Keep brand assets separate from generic icons. Prefer the existing companion PNG assets for Expo app icon/splash until a deliberate metadata sweep is done. For in-app brand/support surfaces, add or mirror bundled local assets under `mobile/assets/brand/` only if the React Native asset pipeline can load them cleanly; otherwise use the semantic icon wrapper as the safe default. The optional store-support surface must remain separate from external payment links and must never imply an unlocked feature.
...
$uri = "https://relay.beacontools.cc/v1/schedule?airport_iata=ZRH&timezone=Europe%2FZurich&display_grace_minutes=120&display_horizon_hours=24&refresh_seconds=3600&install_id=$installId&app_version=0.5.1-smoke&os_family=windows&requested_gui=native&effective_gui=native&source_mode=real&diagnostics_mode=manual"
...
- **macOS current:** `dist/LocalFlight.app` plus versioned `dist/LocalFlight-<version>-macos.zip` + `.sha256` from a normal clean build. Upload the zip/checksum; users extract, move the app to Applications, and use Finder Open once.
- **macOS future:** `dist/LocalFlight-<version>-macos.pkg` + `.sha256` from `--installer` after Developer ID/notarization credentials are installed.
...
- Build the current macOS direct-download artifact on macOS: `python build.py --clean` → attach `LocalFlight-<version>-macos.zip` and `.sha256`.
- Optional future signed path: `python build.py --clean --installer` → attach `LocalFlight-<version>-macos.pkg` and `.sha256` only after signing/notarization succeeds.
...
## Archived session history (do not use as current state)

Everything below this heading is retained only as implementation history. It contains superseded versions, artifact names, test counts, and pending lists. The dated `0.5.1` snapshot at the top of this file and the final release checklist at the end are authoritative.

### Archived macOS / dev-machine handoff

- **macOS handoff focus (2026-07-10 / 0.5.1):** get the Mac onto the exact same source state as this workspace before running the installer or packaging. The current source must include the Beacon Tools public site/docs refresh, mobile/support/network/privacy-choices site pages, default relay `https://relay.beacontools.cc`, mobile Standalone and Remote Companion, Android local dev docs, Matrix integrity work, History movement hardening, AeroDataBox/AviationStack fusion, native/browser polish, macOS icon/package tooling, `assets/previews/` screenshot tree, and the 0.5.1 metadata/docs sweep. If the current workspace still has uncommitted/unpushed native FIDS, radar, Matrix, Settings, installer, relay, mobile, assets, or docs changes, `git pull` on macOS will not see them. Push the branch/commit first, or transfer an explicit patch bundle; otherwise the Mac installer can rebuild an old app with missing Beacon links, wrong relay defaults, missing FIDS details, missing radar ground/context drawings, stale Matrix/settings screens, stale previews, stale docs, or stale version metadata.
...
- Native UI freshness checks on Mac after install/build:
  ...
  - The macOS source installer should be run as `bash installers/macos/install.sh --display native` after the source is verified current. Current release users should use the versioned GitHub app zip, not an older source-built app in `~/Applications`.
- macOS local/dev app repack runbook:
  ```bash
  python build.py --clean
  test -d dist/LocalFlight.app
  test -f dist/LocalFlight-0.5.1-macos.zip
  (cd dist && shasum -a 256 -c LocalFlight-0.5.1-macos.zip.sha256)
...
- Mobile continuation belongs on the Mac/Xcode/Android Studio machine. First run `cd mobile && npm install && npm run verify && npm run a11y && npx expo config --type public`; confirm public Expo config shows version `0.5.1`, iOS bundle ID `cc.beacontools.localflight`, iOS build number `5`, Android package `cc.beacontools.localflight`, Android version code `9`, both widget plugins, and scheme `localflight`. Then run simulator/device smoke with `npm run ios` or `npm run ios:device`; Android local development builds are supported through `npm run android` / `npm run android:device` after Android SDK setup. Companion should prefer the desktop/Pi LAN IP or the native/browser Settings QR; `http://localflight.local:8000` is only a fallback when exactly one Local Flight server is on the LAN. Do not use phone-local `localhost`. Remote Companion smoke must pair on LAN, run the mobile **Test Remote** encrypted probe, block LAN, verify `REMOTE` for Board/Radar/History/Control, revoke, and confirm access stops. Standalone should verify airport search, activation, FIDS, Radar, local History, Settings, and direct relay reporting without a LAN server.
- Mobile store-track continuation on macOS: make sure Apple provisioning covers `cc.beacontools.localflight`, `cc.beacontools.localflight.widget`, and `group.cc.beacontools.localflight`; keep the Google Play app on package `cc.beacontools.localflight`; and use Standalone as the reviewer path so review does not require a desktop/Pi host. Create/approve all three matching consumable support products first, configure relay Apple/Google verification credentials as Fly secrets, and use sandbox/license-test accounts. Build/submit `5`/`9` only after local widget, app, and purchase-recovery smoke: `npx eas build -p ios --profile beta`, `npx eas submit -p ios --profile beta`, `npx eas build -p android --profile beta`, and `npx eas submit -p android --profile beta`.
- IAP relay secrets are operator-only: Apple uses `APPLE_IAP_KEY_ID`, `APPLE_IAP_ISSUER_ID`, and either `APPLE_IAP_PRIVATE_KEY_B64` (preferred for Fly) or `APPLE_IAP_PRIVATE_KEY`, with optional `APPLE_IAP_BUNDLE_ID=cc.beacontools.localflight`; Google uses `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON_B64` (preferred) or `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON`, with optional `GOOGLE_PLAY_PACKAGE_NAME=cc.beacontools.localflight`. Never commit these values, expose them in public docs, add raw purchase evidence to reports, or log request bodies for `/v1/mobile/iap/verify`.
- Network Admin overview receives only aggregate IAP attempt/verified/failure counts plus product/platform/status totals. It intentionally exposes no transaction references, install fingerprints, raw evidence, prices, currencies, or store-account data. Repeated verification failures stay out of Linear and user diagnostics unless a separate sanitized bug is submitted.
...
- Release resume: rebuild Windows, Pi, and the versioned macOS app zip after the current mobile/relay/site/docs tree is committed. The website currently expects `LocalFlight-0.5.1-macos.zip`; old unversioned Mac archives and their checksums are stale. A signed/notarized `.pkg` remains optional future work after Developer ID credentials are available.
...
- macOS packaging has two paths: `python build.py --clean` creates the current ad-hoc-signed, versioned `.app` zip for direct download; `python build.py --clean --installer` remains the future signed/notarized `.pkg` path. Both require native smoke coverage for current History/Matrix/Settings/FIDS/Radar behavior; only the `.pkg` path requires Developer ID Application, Developer ID Installer, and notarytool credentials.
...
- [ ] Repack the current macOS `.app`: `.venv/bin/python build.py --clean`, then verify `dist/LocalFlight.app`, `dist/LocalFlight-0.5.1-macos.zip`, and its checksum. Smoke the extracted archive through Finder Open before upload.
- [ ] Create the GitHub release `v0.5.1` and attach Windows, versioned macOS zip, and Pi artifacts plus all matching `.sha256` files. After Apple Developer ID credentials arrive, the Mac zip may be replaced by the signed/notarized `.pkg` path in a later release.
