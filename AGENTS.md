...
- Desktop and Pi are the public release path for this milestone. Mobile is store-bound after the Remote Companion connectivity/privacy proof pass, with Beacon-owned bundle/package IDs `cc.beacontools.localflight`, URL scheme `localflight`, iOS build number `4`, and Android versionCode `8` for the current `0.5.1` testing upload.
...
- Current macOS/Pi artifacts were rebuilt from the July 12 working tree after the full 549-test/mobile/docs gate. `dist/LocalFlight-macos.zip` is an ARM64 local-testing app archive (ad-hoc signed, not a public notarized package), SHA256 `23128da2fd768d3fb8dfe1dd5b0ea863677563f075347d7dbe09f62f9caa2514`. `dist/LocalFlight-pi-source-0.5.1.zip` SHA256 is `7d0e2bb6c9ad01ddbdbbb8f35dc90dfb7b3d561eeec18d6b4a4c9bc300702653`. The Windows installer and public Developer ID signed/notarized macOS `.pkg` remain to be built on their credentialed platforms before the GitHub release.
- The current mobile build has no payments, support-tip UI, purchase processing, or IAP relay endpoint. Reintroduce monetization only as a separately designed and store-tested feature.
...
- Mobile `0.5.1` signed beta builds completed on EAS from commit `4fd46ad`: iOS build `4` is build ID `1adc2a18-2997-4c5f-ba6d-9052e610e269`; Android versionCode `8` is build ID `b0d2f061-7626-4010-a884-822edecd4a3f`. Do not rebuild or increment these counters for the same testing upload. Submission is pending one-time store linkage only: add the Local Flight App Store Connect numeric `ascAppId` to the EAS submit profile after signing into App Store Connect, and configure the Google Play service-account key through EAS/Play Console. Never commit either store credential or service-account JSON.
