...
- Planned preview asset drop: the user will add an `assets/mobile-previews/` folder under the repo-level `assets/` directory containing Android and iOS device screenshots. Treat those screenshots as documentation/website implementation source assets for mobile preview imagery; prefer using or deriving from them for public docs/site mobile previews instead of inventing fresh mockups when they are available.
