...
## Current 0.2.8 handoff snapshot (2026-05-28)

- Active package target: `0.2.8` as the current beta/client-polish line. `pyproject.toml` remains the source of truth; keep runtime fallbacks, native docs, mobile metadata, preview/release docs, and store-review notes aligned with it.
- Current validation history: after the Beacon relay cutover on 2026-05-22, the focused relay/native suite passed with `392 passed`, compileall passed, mobile `npm run typecheck` passed, and `git diff --check` passed. After the public/docs refresh, the doc/native regression slice passed with `287 passed`, HTML parsing for `site/` and preview pages passed, and Cloudflare served the updated copy. After the Beacon Tools mobile/trust site pass, all `site/**/*.html` parsed, `/assets/*` references resolved, stale URL scans passed, local static routes returned `200`, and Cloudflare served `/local-flight/mobile/`, `/support/`, `/network/`, `/privacy/choices/`, with `/local-flight/privacy` still redirecting to `/privacy`. After Matrix v4 renderer/live-settings/local-clock/web-preview hardening, focused Matrix checks passed and the full Windows/Codex suite returned `432 passed`. The final Windows/Pi docs-and-package sweep passed HTML reference checks, compileall, and full `.venv\Scripts\python.exe -m pytest tests -q` with `441 passed`. On macOS/Codex, `.venv/bin/python build.py --clean` repacked the unsigned local/dev `dist/LocalFlight.app` and `dist/LocalFlight-macos.zip`; `codesign --verify --deep --strict` passed, the zip checksum verified, and `spctl` rejection is expected until Developer ID signing/notarization is available. Treat those as pre-0.2.8 validation/package results until this version sweep's validation and packaging steps are rerun. `npm audit --omit=dev` still reports 4 moderate advisories through Expo's Metro/PostCSS dependency chain, but the suggested `npm audit fix --force` would install a breaking Expo version, so do not force it inside this beta sweep.
- `0.2.8` release sweep state: `pyproject.toml`, runtime fallbacks, relay-facing User-Agent fallbacks, bundled native docs metadata, mobile package/app metadata, store review notes, README/install/display docs, changelog, release notes, site copy, and preview-source wording are aligned to the active 0.2.8 line. Public docs stay DAU-friendly and must not copy internal relay/admin/Fly/Linear runbook detail out of this handoff.
- `0.2.8` validation from this macOS/Codex sweep: `.venv/bin/python -m pytest tests -q` passed with `472 passed`, `.venv/bin/python -m compileall -q src relay installers scripts tests` passed, `cd mobile && npm run verify` passed (`typecheck`, widget contract, Expo Doctor 18/18), and `git diff --check` passed. The six earlier failures were resolved by updating stale clock/gallery/provider assertions, restoring Companion setup copy, adding the Beacon Tools homepage lockup image, and cleaning obsolete site preview assets.
- Mobile private-beta prep is staged but not uploaded: Expo identity now uses `cc.beacontools.localflight` for both iOS bundle ID and Android package, `ios.buildNumber` is `"1"`, `android.versionCode` is `1`, app version is `0.2.8`, URL scheme remains `localflight`, `mobile/eas.json` has a store-distribution `beta` profile, and `mobile/metro.config.js` is intentionally tracked so Expo Doctor stays green. Future dormant support-tip/widget identifiers were moved to the Beacon prefix too. The generated `mobile/android/` native files are ignored/prebuild output; treat `mobile/app.json` and EAS config as the durable source and let Expo/EAS regenerate native Android state on macOS if needed.
- Mobile validation from the earlier Windows/Codex pass: `cd mobile && npm run verify` passed (`typecheck`, widget contract, Expo Doctor 19/19), `npm run a11y` passed, `npx expo config --type public` showed the Beacon bundle/package IDs and build counters, focused store identity + relay mobile IAP tests passed, `compileall` for `relay`/`tests` passed, and `git diff --check` passed with CRLF warnings only. That older full-suite drift has been superseded by the macOS/Codex `472 passed` validation above.
- Windows and Pi artifacts were rebuilt from the final Windows/Codex workspace on 2026-05-22 after the Beacon Tools support forms, Cloudflare Worker route repair, docs sweep, relay SMTP/contact hardening, and Matrix v4 polish. These are now historical `0.2.7` artifacts and must be rebuilt for 0.2.8:
...
- macOS local/dev packaging must be refreshed after the 0.2.8 version sweep before release smoke. Public macOS release packaging still needs the signed/notarized `.pkg` pass after Apple Developer ID credentials are available. Do not reuse stale `0.2.5b5`, `0.2.6`, `0.2.7`, or pre-standalone/pre-Matrix-integrity macOS/Pi handoff notes.
...
- Read at runtime via `importlib.metadata.version("localflight")` with `"0.2.8"` fallback
- Injected as `app_version` Jinja2 global in `server.py` → available in all templates
- Shown in the native footer/brand surfaces and Admin -> System card (`v0.2.8`)
...
- LAN Companion's main bottom nav is `Board`, `Radar`, `History`, and `Control`. `Control` is the safe-remote-actions surface: host health, saved server, airport/source/cadence access through the config sheet, local airport profiles, scheduler restart, diagnostics visibility, Matrix live controls, setup rerun, and Help & Reports. Help remains an internal/support surface, not a bottom-nav destination.
...
$uri = "https://relay.beacontools.cc/v1/schedule?airport_iata=ZRH&timezone=Europe%2FZurich&display_grace_minutes=120&display_horizon_hours=24&refresh_seconds=3600&install_id=$installId&app_version=0.2.8-smoke&os_family=windows&requested_gui=native&effective_gui=native&source_mode=real&diagnostics_mode=manual"
...
- Mobile real-device beta smoke checklist: fresh Standalone setup, LAN Companion manual URL and QR pairing, camera-denied path, iOS local-network-denied path, relay offline/error path, FIDS, Radar, History, Matrix where paired, Settings, manual report/support flow, large text, reduced motion, and readable contrast. Do not widen beyond TestFlight internal / Play internal testing until these pass on real devices.
- Keep public/internal separation intact during the Mac pass: public docs may describe the native privacy-first GUI, public relay behavior, mobile trust pages, and user reporting, but should not expose operator Fly secrets, `DEV_README.md`, `AGENTS.md`, raw relay admin API paths, private admin URLs, Linear team IDs, or admin runbooks.
- Active desktop/server/mobile version is `0.2.8`: `pyproject.toml`, runtime fallbacks, bundled release notes, docs, mobile metadata, and store review notes should all agree unless we deliberately split a later mobile preview version.
- `0.2.8` is no longer preliminary. Treat `docs/release-notes-0.2.8.md` as the active public notes and rebuild artifacts from this source state before publishing.
...
- macOS packaging now has two paths: `python build.py --clean` for local/dev `.app` + zip testing, and `python build.py --clean --installer` for the public signed/notarized `.pkg`. The `.pkg` path verifies `dist/LocalFlight.app`, `dist/LocalFlight-0.2.8-macos.pkg`, and `.sha256`, then needs a native app smoke for latest History/Matrix/Settings/FIDS/Radar behavior. The `.pkg` build intentionally fails without Developer ID Application, Developer ID Installer, and notarytool credentials.
- Mobile package metadata is now aligned to the repo `0.2.8` line. Independent mobile appearance, landscape display behavior, responsive radar, and pinch zoom are implemented in code. Matrix runtime/editor code still exists for LAN Companion but is not part of Standalone.
...
- Mobile detail communication is mode-specific: LAN Companion calls the paired server's `/api/fids/detail` and never calls AviationStack, ADS-B Exchange, RapidAPI, or provider APIs directly. Standalone does not have full detail parity yet; it talks to the hosted relay's mobile endpoints, not raw provider APIs.
...
- [x] Run the 0.2.8 validation sweep from the current tree: full pytest passed with `472 passed`, compileall passed, mobile verify passed, and `git diff --check` passed.
- [ ] Rebuild the Windows artifact on the Windows dev machine from the current 0.2.8 commit: `python build.py --clean`, verify `dist/LocalFlight-windows.zip.sha256`, and confirm the zip carries `localflight-0.2.8.dist-info`.
- [ ] Rebuild the Pi source artifact from the current 0.2.8 commit: `python scripts/package_pi_source.py`, verify `dist/LocalFlight-pi-source-0.2.8.zip.sha256`, and confirm internal handoff docs are excluded.
- [ ] Repack the local/dev macOS `.app` from the current 0.2.8 commit: `.venv/bin/python build.py --clean`, verify `dist/LocalFlight.app`, `dist/LocalFlight-macos.zip`, and `dist/LocalFlight-macos.zip.sha256`.
- [ ] After Apple Developer ID credentials are installed, build the public macOS `.pkg`: `python build.py --clean --installer`, verify signing/notarization, then create the GitHub release `v0.2.8` and attach Windows, macOS pkg, and Pi artifacts plus all matching `.sha256` files.
...
- [ ] Validate LAN Companion runtime flows on-device/simulator: setup, Board/Radar/History/Control, Help & Reports inside Control, appearance persistence, airport/source config sheet, scheduler restart, feedback, crash gating, WebSocket refresh, landscape display, and radar pinch zoom.
