...
**Dev studio/home domain:** Beacon Tools — `beacontools.cc`, owned via Cloudflare at about `$8/year`. Public Local Flight home is `https://beacontools.cc/local-flight`; public privacy policy is `https://beacontools.cc/privacy`; contact is `home@beacontools.cc` and privacy contact is `privacy@beacontools.cc`.
...
- Separation of power still applies: public docs stay user-focused; relay operator/admin details belong only in `AGENTS.md`, `DEV_README.md`, `CLAUDE.md`, and operator tooling. Cloudflare Pages serves the no-build public site from `site/`; edits there only go live after commit + push to `main`.
