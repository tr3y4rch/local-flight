...
function renderOperatorDetail() {
  ...
  if (tab === "summary") {
    ...
    body = `<section class="detail-section"><h3>Support notes</h3><p>Plain text, encrypted at rest, append-only. Add a correction as a new note.</p>${action("Add support note","add_note")}${d.notes.map(n=>`<article class="operator-note"><p>${esc(n.text)}</p><small>${esc(opWhen(n.created_at))}</small></article>`).join("")}</section>`+opHistory(d.history)+opButton("Browse full operator history","license_history")+accessRecordBlock("Purchase transitions",d.purchase_transitions,{heading:r=>`${r.from_state || "New"} → ${r.to_state}`,rows:r=>[["Source",r.source],["Reason",r.reason_code],["Changed",dateTime(r.created_at)]]});
  }
...
async function operatorLicenseAction(action) {
  ...
  const copy = {resend_key_email:"Send the current key to the verified protected address. The key and receiver stay unchanged. A ten-minute cooldown applies.",send_recovery_link:"Send a short-lived recovery/verification link to the bound address. This does not rotate the key.",add_note:"Add a permanent support note. Credentials and email addresses are redacted. Corrections must be new notes.",start_email_change:"Both mailboxes must explicitly confirm within 30 minutes. Only then will ownership change, the key rotate, and the receiver disconnect.",cancel_email_change:"Cancel pending confirmations; existing ownership stays unchanged.",rotate_key:"Invalidate the old key, disconnect active and pending receivers, and email a replacement only to the verified protected address.",revoke_receiver:"Disconnect the receiver. It must activate again; usage is not reset.",suspend_license:"Suspend access and disconnect the receiver.",revoke_license:"Revoke access and disconnect the receiver.",reactivate_license:"Restore eligibility only if purchase or grant authority allows it. Old receiver credentials are not revived."}[action];
  if (!copy) return;
...
async function opGrantWizard(grantId="",action="") {
  ...
  if (duration) fields.unshift({name:"duration",label:"Duration",type:"select",value:issue && test ? "limited" : "permanent",options:[["permanent","Permanent"],["limited","Time-limited"]]}, {name:"expires_at",label:`Expiry date and time (${Intl.DateTimeFormat().resolvedOptions().timeZone})`,type:"datetime-local",value:new Date(Date.now()+7*86400000-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16),hint:"Used only for time-limited grants. Stored as an absolute UTC timestamp."});
